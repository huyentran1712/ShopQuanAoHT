...
if($_SERVER["REQUEST_METHOD"] == "POST")
{
    if(isset($_POST["editSubmit"]))
    {
        ...
        if (!$NameError && !$DescError && !$SaleError && !$Sale_priceError && !$CostError && !$TypeError && !$PriceError) 
        {
            ...
            $product = new Product();
            if ($product->edit($pdo, $_GET['id'], $name, $description, $sale, $sale_price, $cost, $fname . '.' . $extension, $type)) {
...
<div class="container text-center d-flex align-items-center min-vh-200">
    <div class="card mx-auto py-3 text-light" style="width: 40rem; background-color: black">
        <div class="card-title" >
            <p style="font-size: 30px;"><b>THAY ĐỔI SẢN PHẨM</b></p>            
        </div>
    </div>
</div>
<div class="container text-center d-flex align-items-center min-vh-200">
    <div class="card mx-auto py-3" style="width: 40rem">
        <div class="card-body">
            <div class="input-acc">
                <form method="post" action="edit-product.php" class="m-auto" enctype="multipart/form-data">
                    ...
                    <button type = "submit" class = "btn btn-info" name ="editSubmit">Edit Product</button>
                </form>
            </div>
        </div>
    </div>
